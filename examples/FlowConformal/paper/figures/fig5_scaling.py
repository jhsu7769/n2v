"""Figure 5 — Wall-clock vs network parameter count.

x-axis = log(network parameter count); y-axis = log(wall-clock per
instance, s). One line per method, drawing on Exp 1 + Exp 2.

Network parameter counts are hardcoded in this script (would otherwise
need ONNX-load + count). They are approximate, sourced from the
benchmark documentation; tighten them before final publication.
"""
...
def _wall_mean(rows: list[dict[str, str]], wall_key: str) -> float:
    vals = []
    for r in rows:
...
def _exp1_load(csv_dir: Path, bench: str, method: str) -> float:
    if method == "ours":
        return _wall_mean(read_csv_rows(csv_dir / f"exp1_{bench}_ours.csv"), "wall_s")
    if method == "alpha_beta_crown":
        return _vnncomp_wall_mean(csv_dir, bench)
    return _wall_mean(read_csv_rows(csv_dir / f"exp1_{bench}_{method}.csv"), "wall_s")


def _exp2_load(csv_dir: Path, bench: str, method: str) -> float:
    if method == "ours":
        return _wall_mean(read_csv_rows(csv_dir / f"exp2_{bench}_ours.csv"), "wall_s")
    if method == "alpha_beta_crown":
        return _wall_mean(read_csv_rows(csv_dir / f"exp2_{bench}_alpha_beta_crown.csv"), "wall_s")
    return _wall_mean(read_csv_rows(csv_dir / f"exp2_{bench}_{method}.csv"), "wall_s")


def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__)
    add_common_args(parser, default_output="fig5_scaling.png")
    args = parser.parse_args()
    output = args.output or (Path(__file__).resolve().parent / "fig5_scaling.png")
    ...
    ax.set_title("Scaling: wall-clock vs network size")
