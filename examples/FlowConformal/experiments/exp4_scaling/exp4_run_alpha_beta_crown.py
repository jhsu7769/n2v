"""Exp 4 — αβ-CROWN runner on synthetic-network family.

One script invocation iterates the 10 instances at a given depth and
writes one CSV row per instance under
``examples/FlowConformal/experiments/exp4_scaling/outputs/exp4_d<D>_alpha_beta_crown.csv``.

Calls αβ-CROWN via :mod:`examples.FlowConformal.experiments._external_verifiers`
...
Usage::
    ...
    nohup /home/sasakis/miniconda3/envs/n2v/bin/python -u -m \\
        examples.FlowConformal.experiments.exp4_scaling.exp4_run_alpha_beta_crown \\
        --depth 16 \\
        > examples/FlowConformal/experiments/exp4_scaling/outputs/exp4_d16_alpha_beta_crown.log 2>&1 &
    disown

Each instance has ``ground_truth='unsat'`` by construction (1-Lipschitz
networks with specs at empirical-max + 0.1). A ``verdict='SAT'``
indicates a soundness violation in αβ-CROWN — should never happen on
this family, and is what makes Exp 4 a clean signal of soundness.
"""
...
# Custom αβ-CROWN config tuned for our deep ReLU MLP family. See
# ``abcrown_exp4_deep_mlp.yaml`` for the rationale (small batch_size to
# avoid CUDA OOM at depth 24+, no PGD pre-attack since the ground
# truth is UNSAT-by-construction, bab-refine for ReLU MLPs).
_DEEP_MLP_CONFIG = (
    Path(__file__).resolve().parent / 'abcrown_exp4_deep_mlp.yaml'
)
...
def _write_timeout_row(out_csv, depth, instance_idx, timeout_s):
    ...
    with open(out_csv, 'a' if file_exists else 'w', newline='') as f:
        ...
        row.update({'depth': depth, 'instance_idx': instance_idx,
                    'method': 'alpha_beta_crown', 'verdict': 'TIMEOUT',
                    'timeout_s': timeout_s,
...
def main():
    ...
    out_csv = (args.output_csv if args.output_csv is not None
               else _OUT_DIR / f'exp4_d{args.depth}_alpha_beta_crown.csv')
    ...
    print(f'Tool: αβ-CROWN  timeout={_TIMEOUT_S}s', flush=True)
    ...
    if args.smoke:
        ...
        if counts.get('SAT', 0) > 0:
            # Synthetic networks are UNSAT by construction; SAT here is
            # a soundness violation in αβ-CROWN. Almost certainly we
            # have a config / spec encoding bug — flag it loudly.
            print(f'[smoke] FAIL: αβ-CROWN returned SAT on a '
                  f'UNSAT-by-construction instance — likely spec/config '
                  f'bug. counts={counts}', file=sys.stderr)
            sys.exit(2)
        print('[smoke] PASS: αβ-CROWN ran end-to-end without errors.')
