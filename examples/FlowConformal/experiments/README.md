...
| **Per-benchmark hparam overrides** | Original 5 benchmarks locked by lock-probe (5 instances each, status `ok`) at [probes/outputs/probe_amls_bounded_lock.csv](../probes/outputs/probe_amls_bounded_lock.csv): **acasxu_2023: `base` (n_train=5K, flow_epochs=2K, scenario_n=2K, max_levels=30)**; collins_rul_cnn_2022: `small` (1K/1K/500); dist_shift_2023, linearizenn_2024, tllverify_2023: `mega` (10K/2K/2K, max_levels=30). New benchmarks (malbeware, metaroom_2023) use `mega` + `verification_method='amls_bounded_union'` (validated end-to-end at 67s/100s budget for malbeware and 94s/210s budget for metaroom; lock-probe rerun confirms via [probes/outputs/probe_amls_bounded_lock.csv](../probes/outputs/probe_amls_bounded_lock.csv)). |
| **Falsifier** | **ON** for both ours and Hashemi-clipping with **identical** per-benchmark APGD-only budgets (tuned by [`probes/probe_falsifier_budget.py`](../probes/probe_falsifier_budget.py); most benchmarks `(n_restarts=3, n_steps=25)`, `dist_shift_2023` `(5, 50)`). Both call the same [`n2v.utils.falsify.falsify`](../../../n2v/utils/falsify.py) entrypoint with the same kwargs, so the only methodological difference between ours and Hashemi is the score function and calibrated set's geometry. |
| **K seeds** | K=1 |
...
| ✅ Lock-in probe for Exp 1/2 configs ([probe_amls_bounded_lock.csv](../probes/outputs/probe_amls_bounded_lock.csv) and [probe_amls_bounded_lock_overnight.csv](../probes/outputs/probe_amls_bounded_lock_overnight.csv)) | done for the original 5 Exp 1 benchmarks + Exp 2's cifar10_resnet110 (lock probe v1); plus the overnight probe locked vit_2023, cifar100_2024, malbeware, metaroom_2023 at mega + amls_bounded_union. tinyimagenet_2024 verified end-to-end at mega + amls_bounded_union (93s/100s budget) but a full lock probe still owes. |
